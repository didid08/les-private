                                ...
                                @if ($paket->pesertaDidikHasJadwal == null)
                                    <tr class="text-gray-700 dark:text-gray-400">
                                        <td class="px-4 py-3 text-sm">
                                            @csrf
                                            <input type="hidden" name="pesertaDidikHasPaketPembelajaranID" value="{{ $paket->id }}">
                                        ...
                                        <td class="px-4 py-3 text-sm text-center">
                                            <select
                                                ...
                                                @foreach ($semuaJadwalPendidik as $jadwal)
                                                    <option value="{{ $jadwal->id }}">{{ substr($jadwal->hari, 1) }} {{ date('H:i', strtotime($jadwal->pukul_mulai)) }} s/d {{ date('H:i', strtotime($jadwal->pukul_selesai)) }} ( Pendidik: {{ $jadwal->pendidik->nama }} )</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td class="px-4 py-3 text-sm text-center">
